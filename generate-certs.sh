...
services:
  nginx-mtls:
    ...
    volumes:
      ...
      - ./server/server_good.pem:/etc/nginx/certs/server_cert.pem:ro
...
echo "  server_wrong.pem  (EKU: clientAuth — wrong for server)"
echo "  server_noeku.pem  (no EKU — unrestricted)"
echo ""
echo "CLIENT CERTIFICATES:"
echo "  client_good.pem   (EKU: clientAuth)"
echo "  client_wrong.pem  (EKU: serverAuth — wrong for client)"
echo "  client_noeku.pem  (no EKU — unrestricted)"
echo ""
echo "===== START NGINX ====="
echo "  cd mtls-lab && docker compose up -d"
echo ""
echo "===== TEST A: CLIENT CERT EKU (curl -k skips server validation) ====="
echo ""
echo "✅ PASS — client_good.pem (clientAuth):"
echo '  curl -vk https://localhost:8443 --cert client/client_good.pem --key client/client_key.pem'
echo ""
echo "❌ FAIL — client_wrong.pem (serverAuth → wrong EKU):"
echo '  curl -vk https://localhost:8443 --cert client/client_wrong.pem --key client/client_key.pem'
echo ""
echo "✅ PASS — client_noeku.pem (no EKU = unrestricted = accepted):"
echo '  curl -vk https://localhost:8443 --cert client/client_noeku.pem --key client/client_key.pem'
echo ""
echo "===== TEST B: SERVER CERT EKU (use --cacert instead of -k) ====="
echo ""
echo "✅ PASS — server_good.pem (serverAuth):"
echo '  curl -v --cacert ca/ca_cert.pem https://localhost:8443 --cert client/client_good.pem --key client/client_key.pem'
echo ""
echo "To test wrong server EKU, edit docker-compose.yml → swap server_good.pem:"
echo "  ❌ server_wrong.pem (clientAuth) → curl rejects"
echo "  ✅ server_noeku.pem (no EKU)     → curl accepts (unrestricted)"
echo ""
echo "NOTE: curl -k disables ALL server-side validation including EKU checks."
echo "      No EKU on a cert means 'valid for any purpose' per RFC 5280."
echo ""
